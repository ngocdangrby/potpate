...
if(!function_exists('etheme_page_config')) {
	function etheme_page_config() {
		...
		if ( in_array($layout['sidebar'], array('left', 'right')) ) {
...
if(!function_exists('etheme_get_page_id')) {
	function etheme_get_page_id( $singular = false ) {
		...
		if(class_exists('WooCommerce') && (is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() || is_singular( "product" ) || is_tax( "brand" ) || apply_filters('dokan_archive_page', false))) {
			$page = array(
